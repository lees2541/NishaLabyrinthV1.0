#setblock 678 43 575 minecraft:redstone_block replace
execute at @a run playsound minecraft:alarm master @a ~ ~ ~ 3
scoreboard players set r NL_red_progress 1
scoreboard players set r NL_red_progress_left 9
advancement grant @a only nishalabyrinth:system/1st
...
function nishalabyrinth:system/library/open_library

title @a title [{"text":"8개 남았습니다","color":"dark_red","bold":true}]
